...
-export([retrieve_access_token/5, request/7]).

-spec retrieve_access_token(binary(), binary(),
                            binary(), binary(), binary()) -> {ok, map(),
                                                              binary() | map() | list()} |
                                                             {error, any()}.

retrieve_access_token(Type, Url, ID, Secret, Scope) ->
    Client = #{grant_type => Type, auth_url => Url,
               id => ID, secret => Secret, scope => Scope},
    do_retrieve_access_token(Client).

do_retrieve_access_token(#{auth_url := AuthURL,
                           grant_type := <<"password">>} =
                             Client) ->
    Payload = maps:with([grant_type,
                         username,
                         password,
                         scope],
                        Client),
    case rest:request(post,
                      qs,
                      utils:to_list(AuthURL),
                      [200],
                      #{},
                      Payload)
        of
        {ok, _, Headers, Body} ->
            ...
            RefreshToken = maps:get(<<"refresh_token">>,
                                    Body,
                                    undefined),
...
do_retrieve_access_token(#{auth_url := AuthURL,
                           grant_type := <<"client_credentials">>, id := Id,
                           secret := Secret} =
                             Client) ->
...
-spec request(atom(), atom(), list() | binary(),
              [integer()], map(), map() | list() | binary(),
              map()) -> {{ok, integer(), map(),
                          map() | list() | binary()} |
    ...
    case do_request(Method,
        ...
        {{_, 401, _, _}, Client2} ->
            {ok, _RetrHeaders, Client3} =
                do_retrieve_access_token(Client2),
            do_request(Method,
                       Type,
                       Url,
                       Expect,
...
do_request(Method, Type, Url, Expect, Headers, Body,
           Client) ->
...
add_auth_header(Headers,
    ...
    maps:put(<<"Authorization">>,
             <<Prefix/binary, " ", AccessToken/binary>>,
             Headers).
