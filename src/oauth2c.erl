...
-spec retrieve_access_token(binary(), binary(), binary(), binary(), binary()) -> {ok, map(), binary() | map() | list() } | {error, any()}.
...
do_retrieve_access_token(#{ 
  ...
  case rest:request(post, qs, utils:to_list(AuthURL), [200], #{}, Payload) of
    {ok, _, Headers, Body} ->
...
do_retrieve_access_token(#{
  ...
  case rest:request(post, qs, utils:to_list(AuthURL), [200], Header, Payload) of
    {ok, _, Headers, Body} ->
      AccessToken = maps:get(<<"access_token">>, Body),
...
-spec request(atom(), atom(), list() | binary(), list(integer()), map(), map() | list() | binary(), map()) -> 
  {{ok, integer(), map(), map() | list() | binary() } | {error, integer(), map(), map() | list() | binary()} | {error, any()}, map()}.
...
do_request(Method, Type, Url, Expect, Headers, Body, Client) ->
  ...
  {rest:request(Method, Type, binary_to_list(Url), Expect, Headers2, Body), Client}.
...
autorization_prefix(bearer) -> 
  <<"Bearer">>;
autorization_prefix(unsupported) -> 
  <<"token">>.
